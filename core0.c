    ...
    if (command_byte_n == 0b00111111) {
        ...
        switch (command_byte_n >> 4) {
            ...
            case 0b00001010:    // F9-F12
                ...
                break;
            default:            // F0-F4
                update_active_functions((((command_byte_n & 0b00001111) << 1) + ((command_byte_n & 0b00010000) >> 4)),
                                        0, false);
                break;
        }
    }
...
int8_t verify_dcc_message() {
    ...
    uint64_t masked_message_3_bytes = input_bit_buffer & MESSAGE_MASK_3_BYTES;
    if (masked_message_3_bytes == MESSAGE_3_BYTES) number_of_bytes = 3;
    uint64_t masked_message_4_bytes = input_bit_buffer & MESSAGE_MASK_4_BYTES;
    if (masked_message_4_bytes == MESSAGE_4_BYTES) number_of_bytes = 4;
    uint64_t masked_message_5_bytes = input_bit_buffer & MESSAGE_MASK_5_BYTES;
...
void evaluate_message() {
    ...
    int8_t number_of_bytes = verify_dcc_message();
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        do {
            ...
            LOG(3, "level %d measurement %f loop-cond: %f\n", level, measurement,
                measurement - (float) CV_ARRAY_FLASH[171]);
            if (level > max_level) {
